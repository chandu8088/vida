...
/**
 * JUnit test class for KYCDocumentTypeServiceImpl
 */
@ExtendWith({ AemContextExtension.class, MockitoExtension.class })
class KYCDocumentTypeServiceImplTest {

	private final AemContext context = new AemContext();

	private KYCDocumentTypeServiceImpl kycDocumentTypeServiceImpl = new KYCDocumentTypeServiceImpl();
	...
	void setUp() throws Exception {
		...
		PrivateAccessor.setField(kycDocumentTypeServiceImpl, "globalConfig", globalConfig);

		clientConfig = context.getService(ClientConfig.class);
		clientConfig = mock(ClientConfig.class);
		PrivateAccessor.setField(kycDocumentTypeServiceImpl, "clientConfig", clientConfig);

		resolverFactory = context.getService(ResourceResolverFactory.class);
		resolverFactory = mock(ResourceResolverFactory.class);
		PrivateAccessor.setField(kycDocumentTypeServiceImpl, "resolverFactory", resolverFactory);

		replicator = context.getService(Replicator.class);
		replicator = mock(Replicator.class);
		PrivateAccessor.setField(kycDocumentTypeServiceImpl, "replicator", replicator);
	}

	/**
	 * Test method for
	 * {@link com.heromotocorp.vida.core.service.impl.KYCDocumentTypeServiceImpl#getKYCDocumentType()}.
	 * 
	 ...
	 */
	@Test
	void testGetKYCDocumentType() throws UnsupportedRepositoryOperationException, RepositoryException,
			ClientProtocolException, IOException, LoginException {
		Assertions.assertTimeoutPreemptively(Duration.ofSeconds(2), () -> {
			...
			Mockito.when(globalConfig.serverName()).thenReturn("serverName");
			...
			kycDocumentTypeServiceImpl.getKYCDocumentType();
		});
	}

	/**
	 * Test method for
	 * {@link com.heromotocorp.vida.core.service.impl.KYCDocumentTypeServiceImpl#processKYCDocumentType(java.lang.String)}.
	 */
	@Test
	void testProcessKYCDocumentType() {
		Assertions.assertTimeoutPreemptively(Duration.ofSeconds(2), () -> {
			Mockito.mockStatic(EntityUtils.class);
			Mockito.mockStatic(HttpClients.class);
			Mockito.when(globalConfig.postRequestToken()).thenReturn("postRequestToken");
			...
			Mockito.when(resolver.adaptTo(Session.class)).thenReturn(session);
			kycDocumentTypeServiceImpl.getKYCDocumentType();
			kycDocumentTypeServiceImpl.processKYCDocumentType(
