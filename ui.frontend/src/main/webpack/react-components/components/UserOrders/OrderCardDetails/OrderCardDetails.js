...
const OrderCardDetails = (props) => {
  ...
  const handleUploadDocuments = (sfOrderInsuranceId, orderId) => {
    uploadDocsHandler && uploadDocsHandler(sfOrderInsuranceId, orderId);
  };

  const handleTrackDelivery = (event, orderId) => {
    trackDeliveryHandler && trackDeliveryHandler(event, orderId);
  };
              ...
              >
                {(cardData.status === CONSTANT.PURCHASE_STATUS.DRAFT ||
                  ...
                  <>
                    ...
                    {/* Temporary condition for V1 Plus */}
                    {cardData.productName.toLowerCase() === "v1 plus" ? (
                      <></>
                    ) : (
                      (cardData.isPreBookingCancelled === "" ||
                        (!hidePurchaseButton &&
                ...
                {(cardData.status === CONSTANT.PURCHASE_STATUS.KYC_DONE ||
                  ...
                  purchaseTracker && (
                    <button
                      className="btn btn--primary"
                      onClick={(event) =>
                        handleTrackDelivery(event, cardData.orderId)
                      }
                    >
