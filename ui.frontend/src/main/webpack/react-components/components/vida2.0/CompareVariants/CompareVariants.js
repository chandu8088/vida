...
const CompareVariants = (props) => {
  ...
  const isHandleCompareVariant = () => {
    setShowDiffSelected(!showDiffSelected);
    if (isShowDifferentVariant) {
      setShowDifferentVariant(!isShowDifferentVariant);
      const filterData = JSON.parse(JSON.stringify(productDetailsList));
      const productConfigList = JSON.parse(JSON.stringify(productConfig));
