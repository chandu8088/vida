...
const NameTicket = (props) => {
  ...
  useEffect(() => {
    if (testDriveData?.dealerName) {
      setUserLocation(testDriveData?.dealerName);
    }
    if (testDriveData?.date) {
      setUserTestDriveDate(testDriveData?.date);
    }
    if (testDriveData?.timeLabel) {
      setUserTestDriveTime(testDriveData?.timeLabel);
    }
  }, [testDriveData]);
...
NameTicket.propTypes = {
  ...
  ticketDownloadRef: PropTypes.string
};
